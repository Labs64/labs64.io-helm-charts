...
e2e-auth-test:
    ...
    no_scope=$(curl -s -o /dev/null -w '%{http_code}' -X POST -H "Authorization: Bearer $BAD_TOKEN" 'http://gateway.localhost/auditflow/api/v1/audit/publish')
    # Fine-grained: a token holding the READ scope must NOT satisfy a WRITE route.
    READ_TOKEN=$(curl -s -X POST 'http://mock-oidc.localhost/labs64io/token' \
      --data-urlencode 'grant_type=client_credentials' \
      --data-urlencode 'client_id=e2e' --data-urlencode 'client_secret=e2e' \
      --data-urlencode 'scope=audit-event:read' | jq -r '.access_token')
    wrong_scope=$(curl -s -o /dev/null -w '%{http_code}' -X POST -H "Authorization: Bearer $READ_TOKEN" 'http://gateway.localhost/auditflow/api/v1/audit/publish')
    echo "no token    -> $no_token (expected 401)"
    echo "with token  -> $with_token (expected not 401/403)"
    echo "no scope    -> $no_scope (expected 403)"
    echo "wrong scope -> $wrong_scope (expected 403; audit-event:read must not satisfy :write)"
    [ "$no_token" = "401" ] || { echo "FAIL: expected 401 without token"; exit 1; }
    case "$with_token" in 401|403) echo "FAIL: token rejected"; exit 1;; esac
    [ "$no_scope" = "403" ] || { echo "FAIL: expected 403 for no-scope token"; exit 1; }
    [ "$wrong_scope" = "403" ] || { echo "FAIL: expected 403 for read-scope token on write route"; exit 1; }
