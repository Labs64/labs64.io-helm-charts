...
| enabled | bool | `true` |  |
| images | object | `{"busybox":"busybox:1.36","curl":"curlimages/curl:8.10.1","postgresql":"postgres:17-alpine","redis":"redis:7-alpine"}` | Images used by the check containers |
| images.postgresql | string | `"postgres:17-alpine"` | Official images rather than bitnamilegacy/*, which is an unsupported namespace on a deprecation path. These run the same SELECT 1 / PING checks; both put psql / redis-cli on PATH, which is all job.yaml relies on. |
