...
| applicationYaml | object | `{"labs64":{"auditflow":{"enabled":false,"oauth2":{"client-id":"payment-gateway","client-secret":"${AUDITFLOW_OAUTH_CLIENT_SECRET}","scopes":["audit-event:write"],"token-uri":"http://keycloak.tools.svc.cluster.local/realms/labs64io/protocol/openid-connect/token"},"source-system":"labs64.io-payment-gateway","url":"{{ if and .Values.global .Values.global.gateway .Values.global.gateway.internalHost }}http://{{ .Values.global.gateway.internalHost }}/auditflow/api/v1{{ else }}${AUDITFLOW_URL}{{ end }}"}},"payment-gateway":{"idempotency":{"processing-timeout":"${IDEMPOTENCY_PROCESSING_TIMEOUT:PT5M}","redis-ttl":"${IDEMPOTENCY_REDIS_TTL:PT3H}"}},"spring":{"data":{"redis":{"host":"{{ if and .Values.global .Values.global.redis }}{{ .Values.global.redis.host | default \"redis.tools.svc.cluster.local\" }}{{ else }}redis.tools.svc.cluster.local{{ end }}","port":"{{ if and .Values.global .Values.global.redis }}{{ .Values.global.redis.port | default 6379 }}{{ else }}6379{{ end }}"}},"datasource":{"url":"jdbc:postgresql://{{ if and .Values.global .Values.global.postgresql }}{{ .Values.global.postgresql.host | default \"postgresql.tools.svc.cluster.local\" }}:{{ .Values.global.postgresql.port | default 5432 }}{{ else }}postgresql.tools.svc.cluster.local:5432{{ end }}/payment_gateway"}}}` | Additional application properties |
| applicationYaml.labs64.auditflow.enabled | bool | `false` | Publish payment lifecycle events through AuditFlow |
| applicationYaml.labs64.auditflow.oauth2.client-id | string | `"payment-gateway"` | OAuth2 service-principal client ID |
| applicationYaml.labs64.auditflow.oauth2.scopes | list | `["audit-event:write"]` | Scopes requested for AuditFlow publishing |
| applicationYaml.labs64.auditflow.oauth2.token-uri | string | `"http://keycloak.tools.svc.cluster.local/realms/labs64io/protocol/openid-connect/token"` | OAuth2 token endpoint used for the client_credentials grant |
| applicationYaml.labs64.auditflow.source-system | string | `"labs64.io-payment-gateway"` | Stable producer identity written into every audit event |
| applicationYaml.labs64.auditflow.url | string | `"{{ if and .Values.global .Values.global.gateway .Values.global.gateway.internalHost }}http://{{ .Values.global.gateway.internalHost }}/auditflow/api/v1{{ else }}${AUDITFLOW_URL}{{ end }}"` | AuditFlow API base URL derived from global.gateway.internalHost; standalone installs may supply AUDITFLOW_URL instead |
| applicationYaml.payment-gateway | object | `{"idempotency":{"processing-timeout":"${IDEMPOTENCY_PROCESSING_TIMEOUT:PT5M}","redis-ttl":"${IDEMPOTENCY_REDIS_TTL:PT3H}"}}` | Payment Gateway application properties bound under the Spring prefix `payment-gateway` |
| applicationYaml.payment-gateway.idempotency | object | `{"processing-timeout":"${IDEMPOTENCY_PROCESSING_TIMEOUT:PT5M}","redis-ttl":"${IDEMPOTENCY_REDIS_TTL:PT3H}"}` | Generic HTTP idempotency storage and processing recovery |
| applicationYaml.payment-gateway.idempotency.processing-timeout | string | `"${IDEMPOTENCY_PROCESSING_TIMEOUT:PT5M}"` | Time after which another identical request may take over an abandoned PROCESSING record |
| applicationYaml.payment-gateway.idempotency.redis-ttl | string | `"${IDEMPOTENCY_REDIS_TTL:PT3H}"` | Lifetime of completed and in-progress idempotency records in Redis |
| applicationYaml.spring | object | `{"data":{"redis":{"host":"{{ if and .Values.global .Values.global.redis }}{{ .Values.global.redis.host | default \"redis.tools.svc.cluster.local\" }}{{ else }}redis.tools.svc.cluster.local{{ end }}","port":"{{ if and .Values.global .Values.global.redis }}{{ .Values.global.redis.port | default 6379 }}{{ else }}6379{{ end }}"}},"datasource":{"url":"jdbc:postgresql://{{ if and .Values.global .Values.global.postgresql }}{{ .Values.global.postgresql.host | default \"postgresql.tools.svc.cluster.local\" }}:{{ .Values.global.postgresql.port | default 5432 }}{{ else }}postgresql.tools.svc.cluster.local:5432{{ end }}/payment_gateway"}}` | Spring configuration |
...
| applicationYaml.spring.datasource.url | string | `"jdbc:postgresql://{{ if and .Values.global .Values.global.postgresql }}{{ .Values.global.postgresql.host | default \"postgresql.tools.svc.cluster.local\" }}:{{ .Values.global.postgresql.port | default 5432 }}{{ else }}postgresql.tools.svc.cluster.local:5432{{ end }}/payment_gateway"` | JDBC URL; resolves to the bundled subchart when postgresql.enabled, else set your database URL Username/password are NOT set here — this block renders into a plain ConfigMap. Set credentials via `secrets.data.SPRING_DATASOURCE_USERNAME`/`SPRING_DATASOURCE_PASSWORD` (a Secret, injected via envFrom) instead — see `secrets.data` below. |
...
| gateway.routes | list | `[{"path":"/api/v1","port":8080,"stripPath":true},{"path":"/v3/api-docs","port":8080,"public":true,"stripPrefix":true}]` | Routes exposed by this module. The whole API surface is ONE route through ForwardAuth: the authproxy is the PEP and derives public-vs-protected per operation from OpenAPI OAuth security plus x-labs64.auth (generated into the authproxy routes manifest + the central Cerbos PDP). Public operations (payment-definitions, PSP webhooks, provider/checkout redirect returns — none can carry a JWT) are allowed without a token by the authproxy itself, so they MUST NOT be re-declared here. Re-declaring them as edge routes duplicates the OpenAPI PEP and mis-strips their paths. OpenAPI stays the single source of truth. |
| gateway.routes[0] | object | `{"path":"/api/v1","port":8080,"stripPath":true}` | Whole Payment Gateway API (ForwardAuth PEP decides per-op from OpenAPI; strips '<prefix>/api/v1' — backend is root-mapped) |
| gateway.routes[1] | object | `{"path":"/v3/api-docs","port":8080,"public":true,"stripPrefix":true}` | OpenAPI docs (springdoc endpoint — NOT an OpenAPI operation, so it has no generated OpenAPI auth policy at the authproxy; public at the edge, prefix stripped) |
...
| networkPolicy | object | `{"egress":[{"ports":[{"port":3593,"protocol":"TCP"}],"to":[{"podSelector":{"matchLabels":{"app.kubernetes.io/name":"authz-pdp"}}}]}],"enabled":false,"extraIngress":[],"gatewayNamespace":"tools","ingressControllerLabels":{},"observabilityNamespace":"monitoring","toolsEgress":[{"name":"postgresql","port":5432},{"name":"redis","port":6379},{"name":"traefik","port":8000},{"name":"keycloak","port":8080}]}` | NetworkPolicy: allow ingress from Traefik and same-namespace pods only (rendered by chart-libs.networkpolicy) |
...
| networkPolicy.toolsEgress | list | `[{"name":"postgresql","port":5432},{"name":"redis","port":6379},{"name":"traefik","port":8000},{"name":"keycloak","port":8080}]` | Tools-namespace destinations this service needs (name + port pairs); rendered as scoped egress rules — NOT a blanket allow to the whole tools namespace — to preserve database-per-service isolation. |
| nodeSelector | object | `{}` |  |
...
| secrets | object | `{"data":{}}` | Secret data to be used as environment variables (delivered via envFrom). External installs supply OAuth/database/cache credentials here, e.g.   AUDITFLOW_OAUTH_CLIENT_SECRET, SPRING_DATASOURCE_USERNAME / SPRING_DATASOURCE_PASSWORD, SPRING_DATA_REDIS_PASSWORD. Keys you set here take precedence over the bundled-dep keys. On helm upgrade the Secret is deleted and recreated (hook-managed). Note: the Secret is hook-managed (pre-install) and survives helm uninstall. @schema type: object properties:   data:     type: object     additionalProperties: true @schema |
...
| startupProbe | object | `{"failureThreshold":60,"httpGet":{"path":"/actuator/health/liveness","port":8080},"periodSeconds":2,"timeoutSeconds":2}` | Startup probe (rendered by chart-libs.startupProbe): guards slow cold starts (Spring Boot + OTel Java Agent) so the liveness probe never kills a still-booting pod. Max boot budget = failureThreshold * periodSeconds. |
