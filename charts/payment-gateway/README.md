...
| applicationYaml | object | `{"labs64":{"auditflow":{"oauth2":{"client-id":"payment-gateway","client-secret":"${AUDITFLOW_OAUTH_CLIENT_SECRET}","scopes":["audit-event:write"],"token-uri":"http://keycloak.tools.svc.cluster.local/realms/labs64io/protocol/openid-connect/token"},"source-system":"labs64.io-payment-gateway","url":"{{ if and .Values.global .Values.global.gateway .Values.global.gateway.internalHost }}http://{{ .Values.global.gateway.internalHost }}/auditflow/api/v1{{ else }}${AUDITFLOW_URL}{{ end }}"}},"spring":{"data":{"redis":{"host":"{{ if and .Values.global .Values.global.redis }}{{ .Values.global.redis.host | default \"redis.tools.svc.cluster.local\" }}{{ else }}redis.tools.svc.cluster.local{{ end }}","port":"{{ if and .Values.global .Values.global.redis }}{{ .Values.global.redis.port | default 6379 }}{{ else }}6379{{ end }}"}},"datasource":{"url":"jdbc:postgresql://{{ if and .Values.global .Values.global.postgresql }}{{ .Values.global.postgresql.host | default \"postgresql.tools.svc.cluster.local\" }}:{{ .Values.global.postgresql.port | default 5432 }}{{ else }}postgresql.tools.svc.cluster.local:5432{{ end }}/payment_gateway"}}}` | Additional application properties |
