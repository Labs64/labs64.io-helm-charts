...
| gateway.routes[0] | object | `{"path":"/api/v1","port":8080,"roles":["admin-role","ecommerce-role","default-roles-labs64io"],"stripPath":true}` | Payment Gateway API (protected; Traefik owns the /api/v1 version prefix and strips the full '<prefix>/api/v1' before forwarding — the backend is root-mapped) |
