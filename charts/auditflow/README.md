...
| applicationYaml.spring.datasource.url | string | `"jdbc:postgresql://{{ if and .Values.global .Values.global.postgresql }}{{ tpl (.Values.global.postgresql.host | default \"postgresql.tools.svc.cluster.local\") . }}:{{ .Values.global.postgresql.port | default 5432 }}{{ else }}postgresql.tools.svc.cluster.local:5432{{ end }}/auditflow"` | JDBC URL; resolves to the bundled subchart when postgresql.enabled, else set your database URL |
