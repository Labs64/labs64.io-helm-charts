...
![Version: 0.12.1](https://img.shields.io/badge/Version-0.12.1-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 0.0.11](https://img.shields.io/badge/AppVersion-0.0.11-informational?style=flat-square)
...
| applicationYaml | object | `{"auditflow":{"idempotency":{"store":"redis"},"pipelines":[],"redaction":{"enabled":false,"mask":"***","rules":[]}},"secretRef":{"resolver":"k8s-secret"},"sink":{"discovery":{"mode":"local"},"local":{"url":"http://localhost:8082"},"service":{"name":"auditflow-sink","namespace":"default"}},"spring":{"autoconfigure":{"exclude":[]},"data":{"redis":{"host":"{{ if and .Values.global .Values.global.redis }}{{ .Values.global.redis.host | default \"redis.tools.svc.cluster.local\" }}{{ else }}redis.tools.svc.cluster.local{{ end }}","port":"{{ if and .Values.global .Values.global.redis }}{{ .Values.global.redis.port | default 6379 }}{{ else }}6379{{ end }}"}},"datasource":{"url":"jdbc:postgresql://{{ if and .Values.global .Values.global.postgresql }}{{ .Values.global.postgresql.host | default \"postgresql.tools.svc.cluster.local\" }}:{{ .Values.global.postgresql.port | default 5432 }}{{ else }}postgresql.tools.svc.cluster.local:5432{{ end }}/auditflow"},"rabbitmq":{"host":"{{ if and .Values.global .Values.global.rabbitmq }}{{ .Values.global.rabbitmq.host | default \"rabbitmq.tools.svc.cluster.local\" }}{{ else }}rabbitmq.tools.svc.cluster.local{{ end }}","port":"{{ if and .Values.global .Values.global.rabbitmq }}{{ .Values.global.rabbitmq.port | default 5672 }}{{ else }}5672{{ end }}"}},"tenants":{"consumer":{"max-in-flight-per-tenant":4},"ratelimit":{"backend":"redis"},"source":{"mode":"gitops-configmap"}},"transformer":{"discovery":{"mode":"local"},"local":{"url":"http://localhost:8081"},"service":{"name":"auditflow-transformer","namespace":"default"}}}` | Additional application properties |
| applicationYaml.auditflow | object | `{"idempotency":{"store":"redis"},"pipelines":[],"redaction":{"enabled":false,"mask":"***","rules":[]}}` | AuditFlow application properties (bound by @ConfigurationProperties(prefix = "auditflow")) |
| applicationYaml.auditflow.idempotency | object | `{"store":"redis"}` | Idempotency/dedup store. "redis" (default) is required for more than one replica; "memory" is single-replica only and lets a self-hoster skip running Redis entirely — pair it with `tenants.ratelimit.backend: in-memory` below and the autoconfigure exclude further down, or Redis connection beans still get created even though nothing uses them. |
...
| applicationYaml.auditflow.redaction | object | `{"enabled":false,"mask":"***","rules":[]}` | PII de-identification, applied at ingest before publish. Global (not per-pipeline) — a rule matches the field across every tenant's events. Disabled by default. |
| applicationYaml.auditflow.redaction.enabled | bool | `false` | Master switch — off by default |
| applicationYaml.auditflow.redaction.mask | string | `"***"` | Replacement value used by the "mask" action |
| applicationYaml.auditflow.redaction.rules | list | `[]` | One entry per field to redact: {field (dot/array path, e.g. extra.userEmail), action (mask | hash | drop)} @schema type: array items:   type: object   additionalProperties: true @schema |
...
| applicationYaml.spring | object | `{"autoconfigure":{"exclude":[]},"data":{"redis":{"host":"{{ if and .Values.global .Values.global.redis }}{{ .Values.global.redis.host | default \"redis.tools.svc.cluster.local\" }}{{ else }}redis.tools.svc.cluster.local{{ end }}","port":"{{ if and .Values.global .Values.global.redis }}{{ .Values.global.redis.port | default 6379 }}{{ else }}6379{{ end }}"}},"datasource":{"url":"jdbc:postgresql://{{ if and .Values.global .Values.global.postgresql }}{{ .Values.global.postgresql.host | default \"postgresql.tools.svc.cluster.local\" }}:{{ .Values.global.postgresql.port | default 5432 }}{{ else }}postgresql.tools.svc.cluster.local:5432{{ end }}/auditflow"},"rabbitmq":{"host":"{{ if and .Values.global .Values.global.rabbitmq }}{{ .Values.global.rabbitmq.host | default \"rabbitmq.tools.svc.cluster.local\" }}{{ else }}rabbitmq.tools.svc.cluster.local{{ end }}","port":"{{ if and .Values.global .Values.global.rabbitmq }}{{ .Values.global.rabbitmq.port | default 5672 }}{{ else }}5672{{ end }}"}}` | Spring configuration |
| applicationYaml.spring.autoconfigure | object | `{"exclude":[]}` | Spring Boot auto-configuration classes to exclude. Empty by default (Redis auto-config stays active, matching `idempotency.store: redis` above). For a single-replica, no-Redis deployment, set both `auditflow.idempotency.store: memory` and `tenants.ratelimit.backend: in-memory` above, and list these four classes here so Spring Boot never creates a Redis connection it won't use — this is the exact list the bundled docker-compose dev stack already runs with: exclude:   - org.springframework.boot.data.redis.autoconfigure.DataRedisAutoConfiguration   - org.springframework.boot.data.redis.autoconfigure.DataRedisReactiveAutoConfiguration   - org.springframework.boot.data.redis.autoconfigure.health.DataRedisHealthContributorAutoConfiguration   - org.springframework.boot.data.redis.autoconfigure.health.DataRedisReactiveHealthContributorAutoConfiguration |
...
| applicationYaml.spring.datasource.url | string | `"jdbc:postgresql://{{ if and .Values.global .Values.global.postgresql }}{{ .Values.global.postgresql.host | default \"postgresql.tools.svc.cluster.local\" }}:{{ .Values.global.postgresql.port | default 5432 }}{{ else }}postgresql.tools.svc.cluster.local:5432{{ end }}/auditflow"` | JDBC URL; resolves to the bundled subchart when postgresql.enabled, else set your database URL Username/password are NOT set here — this block renders into a plain ConfigMap. Set credentials via `secrets.data.SPRING_DATASOURCE_USERNAME`/`SPRING_DATASOURCE_PASSWORD` (a Secret, injected via envFrom) instead — see `secrets.data` below. |
...
| image | object | `{"digest":"sha256:15ca8794722d774a317cfeb2a4ca5e5e0d61ab6f72d5bf7d243bf3d8a31709ba","pullPolicy":"IfNotPresent","repository":"labs64/auditflow","tag":""}` | This sets the container image more information can be found here: https://kubernetes.io/docs/concepts/containers/images/ |
| image.digest | string | `"sha256:15ca8794722d774a317cfeb2a4ca5e5e0d61ab6f72d5bf7d243bf3d8a31709ba"` | Pin the image by digest (`sha256:<64 hex>`). Takes precedence over `tag` — the release pipeline sets it so a deployment cannot follow a moved tag. |
...
| sink.image.digest | string | `"sha256:2b75550bf70e375c1ee860c86048fc713ab467d05512c4dd2d7fc0d8f271ff47"` | Pin the image by digest (`sha256:<64 hex>`). Takes precedence over `tag` — the release pipeline sets it so a deployment cannot follow a moved tag. |
...
| transformer.image.digest | string | `"sha256:1bfba556ad2ba3a61387166c3c3b4659181ea26f63b5f7b50b41af5e6b34468d"` | Pin the image by digest (`sha256:<64 hex>`). Takes precedence over `tag` — the release pipeline sets it so a deployment cannot follow a moved tag. |
