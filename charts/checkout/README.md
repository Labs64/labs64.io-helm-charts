...
| applicationYaml.spring.datasource.url | string | `"jdbc:postgresql://{{ if and .Values.global .Values.global.postgresql }}{{ tpl (.Values.global.postgresql.host | default \"postgresql.tools.svc.cluster.local\") . }}:{{ .Values.global.postgresql.port | default 5432 }}{{ else }}postgresql.tools.svc.cluster.local:5432{{ end }}/checkout"` | JDBC URL; resolves to the bundled subchart when postgresql.enabled, else set your database URL Username/password are NOT set here — this block renders into a plain ConfigMap. Set credentials via `secrets.data.SPRING_DATASOURCE_USERNAME`/`SPRING_DATASOURCE_PASSWORD` (a Secret, injected via envFrom) instead — see `secrets.data` below. |
